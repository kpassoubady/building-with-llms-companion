...
Run: python solutions/ch14/eval_harness.py  (from the repo root)
"""
...
def evaluate_response(response, test_case):
    ...
    forbidden_hit = any(
        f.lower() in lower for f in test_case.get("forbidden_keywords", [])
    )
...
def check_for_drift(results_a, results_b, threshold=0.05):
    ...
    if drop > threshold:
        print(
            f"DRIFT DETECTED: pass rate dropped {drop:.1%} "
            f"({rate_a:.1%} -> {rate_b:.1%})"
        )
        return True
