...
Goal: Embed 20 sentences covering different topics, build a full cosine-similarity
matrix with numpy, and surface the most similar sentence pairs and natural clusters.
...
Instructions:
1. Run embed_sentences() to get vectors for all SENTENCES.
2. Implement cosine_similarity_matrix() to return an NxN numpy array of pairwise scores.
3. Implement find_top_pairs() to scan the upper triangle and return the highest-scoring pairs.
4. Run main() and look at the printed pairs - do the clusters match your intuition?
5. Bonus: add 2 more sentences on a new topic and re-run to see where they land.

Run: python exercises/ch10/embedding_explorer.py  (from the repo root)
"""
...
def embed_sentences(sentences):
    ...
    Returns:
        numpy array of shape (N, D) where D is the embedding dimension.
    """
    # TODO: Call litellm.embedding() with EMBEDDING_MODEL and sentences as input.
    #   Extract the embedding vectors from resp["data"] and return as np.array (float32).
    # Hint:
...
def cosine_similarity_matrix(vectors):
    ...
    Returns:
        numpy array of shape (N, N) with scores in [-1, 1].
    """
    # TODO: Normalise each row to unit length, then compute the dot-product matrix.
    #   np.linalg.norm with keepdims=True is useful for normalisation.
    # Hint:
...
def find_top_pairs(sim_matrix, sentences, top_n=10):
    ...
    Args:
        sim_matrix: NxN numpy array of cosine scores.
        sentences: List of sentence strings (same order as matrix rows).
        top_n: How many pairs to return.

    Returns:
        List of (score, sentence_i, sentence_j) tuples, sorted descending by score.
    """
    # TODO: Iterate over the upper triangle (i < j) to avoid duplicates.
    #   Collect (score, i, j) tuples, sort descending, return the top_n.
    # Hint:
    # pairs = []
    # n = len(sentences)
    # for i in range(n):
    ...
    # return pairs[:top_n]
    raise NotImplementedError("Implement find_top_pairs()")
...
# Chapter 10 - Embedding Explorer
#
# Embedding 20 sentences with text-embedding-3-small...
# Vectors shape: (20, 1536)
#
# Top 10 most similar sentence pairs:
# ----------------------------------------------------------------------
#   0.921  'Sauteing vegetables in olive oil brings out their...'
#          'Caramelization occurs when sugars are heated to hi...'
#   0.908  'Neural networks learn by adjusting weights through...'
#          'Gradient descent minimizes the loss function durin...'
#   ...
#
# Sentences with a neighbor scoring >= 0.70:
#   [00] Python uses indentation to define code blocks.
#        -> A function in Python is defined with the def key...
#   ...
