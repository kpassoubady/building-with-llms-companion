...
Run: python exercises/ch08/eval_harness.py  (from the repo root)
"""
...
def score_response(response: str, expected_category: str) -> bool:
    ...
    """
    # TODO: Normalise both strings (strip whitespace, lowercase) and check
    #   whether expected_category appears anywhere in the response.
    #   A substring match is intentionally forgiving - the model sometimes
    #   adds punctuation or a brief explanation.

    # Hint:
    # return expected_category.lower() in response.strip().lower()

    raise NotImplementedError("Complete score_response() to continue.")
...
def run_eval(prompt_fn, dataset=GOLDEN_DATASET):
    ...
    """
    # TODO:
    #   1. Iterate over dataset entries.
    #   2. Call prompt_fn(entry["input"]) to get the model's response.
    #   3. Call score_response() to determine pass/fail.
    #   4. Collect failing cases as dicts with 'input', 'expected', 'actual'.
    #   5. Return (correct / total, failing_cases).
    ...
    raise NotImplementedError("Complete run_eval() to continue.")
