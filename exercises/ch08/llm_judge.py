...
Run: python exercises/ch08/llm_judge.py  (from the repo root)
"""
...
def build_judge_prompt(question: str, answer: str, rubric: str) -> str:
    ...
    The prompt must instruct the model to reply with exactly:
      Score: <integer 1-5>
    ...
    """
    # TODO: Combine question, answer, and rubric into a prompt that clearly
    #   asks for the Score: / Justification: format so parse_score() works.
    ...
    raise NotImplementedError("Complete build_judge_prompt() to continue.")
...
def parse_score(response: str) -> int:
    ...
    """
    # TODO: Use re.search to find a digit following "Score:" in the response.
    #   Return int(match.group(1)) on success, 0 on failure.

    # Hint:
    # match = re.search(r"Score:\s*([1-5])", response, re.IGNORECASE)
    # return int(match.group(1)) if match else 0

    raise NotImplementedError("Complete parse_score() to continue.")
...
def judge(question: str, answer: str, rubric: str):
    ...
    """
    # TODO: Build the prompt with build_judge_prompt(), call get_completion
    #   with tier="default" and temperature=0.0, then parse the score.
    ...
    raise NotImplementedError("Complete judge() to continue.")
